...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { href: `/${locale}/dashboard/events`, icon: <Calendar className="w-5 h-5" />, label: isRu ? "Мероприятия" : isEn ? "Events" : "Evenimente" },
    { href: `/${locale}/dashboard/team`, icon: <Users className="w-5 h-5" />, label: isRu ? "Команда" : isEn ? "Team" : "Echipă" },
    { href: `/${locale}/dashboard/content`, icon: <FileText className="w-5 h-5" />, label: isRu ? "Контент" : isEn ? "Content" : "Conținut" },
  ];
      ...
      <aside className="w-64 bg-blue-900 text-white flex flex-col shrink-0">
        ...
        <div className="p-4 border-t border-blue-800 space-y-2">
          ...
          >
            ...
            {isRu ? "На сайт" : isEn ? "Back to site" : "Înapoi la site"}
          </Link>
          <button
          ...
          >
            ...
            {isRu ? "Выход" : isEn ? "Log out" : "Deconectare"}
          </button>
        </div>
      </aside>
